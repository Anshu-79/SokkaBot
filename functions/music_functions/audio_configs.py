...
class Video:
  ...
  def __init__(self, url_or_search, requested_by):
    ...
    with ytdl.YoutubeDL(YTDL_OPTS) as ydl:
      ...
      get_thumbnail = lambda x : x[-1] #function to get the highest res thumbnail as it's always the last one...if
      self.thumbnail = get_thumbnail(video["thumbnails"])['url']
      #gets the url of thumbnail with get_thumbnail()
      
      get_duration = lambda t : str(t//60) + ' minutes ' + str(t % 60) + ' seconds' if (t >= 60) else str(t) + ' seconds'
